...
```mermaid
flowchart TD
  Submit["Search for misty Wayanad landscapes, tea estates"] --> Outcome{"What appears?"}
...
2. Remove one descriptive phrase. For example, try `Wayanad tea estates`.
...
<Steps>
  ...
  <Step title="Submit one identifiable search">
    In AI Search, submit `misty Wayanad landscapes, tea estates` once. Use the request start time to identify the new search request in the Network list.
  </Step>
...
Query: misty Wayanad landscapes, tea estates
